...
<footer> <!-- Bagian footer -->
  ...
  <p id="waktu"></p> <!-- Tempat menampilkan jam real-time -->
</footer>
<script>
  function updateTime() {                         // Fungsi untuk update waktu
    const now = new Date();                       // Ambil waktu saat ini
    const waktu = document.getElementById("waktu"); // Ambil elemen <p id="waktu">
    waktu.textContent = "Waktu sekarang: " + now.toLocaleTimeString(); // Tampilkan jam
  }
  setInterval(updateTime, 1000);                  // Jalankan setiap 1 detik
  updateTime();                                   // Panggil sekali saat halaman dibuka
</script>
<style>
  footer {
    position: fixed;   /* Biar selalu menempel di bawah layar */
    bottom: 0;         /* Ditempatkan paling bawah */
    left: 0;           /* Ditempatkan mulai dari sisi kiri */
    width: 100%;       /* Lebarnya penuh sepanjang layar */
    text-align: center;/* Teks rata tengah */
    background: #222;  /* Warna latar hitam keabu-abuan */
    color: white;      /* Teks warna putih */
    padding: 10px 0;   /* Ruang atas & bawah biar tidak mepet */
    font-family: Arial, sans-serif; /* Font sederhana & rapi */
